...
function deleteProduct() {
  ...
  // Konfirmasi hapus
  const ok = confirm(`Apakah Anda yakin ingin menghapus?\n\nNama: ${product.name}\nKategori: ${product.category}`);

  if (!ok) return; // batal jika user klik "Cancel"

  products.splice(idx, 1);  // hapus dari array
  save();                   // simpan ke localStorage
  renderDelete();           // update dropdown
